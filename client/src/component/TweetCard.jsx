...
const TweetCard = ({ tweet, className = "" }) => {
  ...
  const tweetLikeState = useSelector(selectTweetLikeState, _id);
  const isLiking = useSelector(selectIsLiking);
  const isLoggedIn = useSelector(selectIsLoggedIn);

  console.log("TweetCard Props:", {
    isLiked,
    likeCount,
  });
  ...
  }, [dispatch, _id, isLiked, likeCount]);
  ...
  const handleDislikeToggle = () => {
    // if (onDislikeToggle) {
    //   onDislikeToggle(_id, !isDisliked);
    // }
  };
        ...
        <div className="flex gap-4">
          ...
          <button
            className={`group inline-flex items-center gap-x-1 outline-none after:content-[attr(data-dislike-count)] focus:after:content-[attr(data-dislike-count-alt)] ${
              isDisliked ? "text-[#ae7aff]" : "text-inherit"
            }`}
