...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something wwent wrong while generating access and refresh tokens"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const oldAvatarPublicId =
    oldAvatarParts[oldAvatarParts.length - 1].split(".")[0];
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  const oldCoverImagePublicId =
    oldCoverImageParts[oldCoverImageParts.length - 1].split(".")[0];
